...
```bash
python analyze.py  --portfolio-csv './data/sample_portfolio.csv'  --portfolio-csv-format 'prices'  --benchmark-csv './data/sample_benchmark.csv' --benchmark-csv-format 'prices'
```

### Run a portfolio built from tickers (equal weights) vs a benchmark ticker
```bash
python analyze.py  --portfolio-tickers "AAPL,MSFT,SPY"  --benchmark-ticker 'SPY'
```

### Run a portfolio built from tickers with explicit weights
```bash
python analyze.py  --portfolio-tickers "SPY,TLT,GLD"  --portfolio-weights "SPY=0.6,TLT=0.3,GLD=0.1"  --benchmark-ticker 'SPY'
```
...
```bash
python analyze.py  --portfolio-tickers "AAPL,MSFT,SPY"  --benchmark-ticker 'SPY'  --start '2018-01-01'  --cache-days 5  --outdir 'output'  --show-plots
```

**3) Build a portfolio from tickers with explicit weights**
```bash
python analyze.py   --portfolio-tickers "SPY,TLT,GLD"   --portfolio-weights "SPY=0.6,TLT=0.3,GLD=0.1"   --benchmark-ticker 'SPY'   --start '2018-01-01' 
```

**4) Optional: PyFolio tear sheet**
```bash
python analyze.py   --portfolio-tickers "MSTR,NVDA,AAPL"   --benchmark-ticker SPY   --start 2018-01-01  --pyfolio --pyfolio-target NVDA --pyfolio-out "output/pyfolio.html"
```
